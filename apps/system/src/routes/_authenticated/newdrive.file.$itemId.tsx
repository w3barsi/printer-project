...
export const Route = createFileRoute("/_authenticated/newdrive/file/$itemId")({
  ...
  loader: async ({ context: { queryClient: qc }, params }) => {
    ...
    return {
      file,
      crumb: [
        { value: "New Drive", href: "/newdrive", type: "static" },
        ...(file
          ? [
              {
                value: file.spaceName,
                href: `/newdrive/${file.spaceId}`,
                type: "static",
              },
      ...
      <main className="grid min-h-[calc(100svh-6.6rem)] gap-3 lg:grid-cols-[300px_minmax(0,1fr)] lg:gap-5">
        <aside className="flex flex-col rounded-xl border bg-card/70 p-4 shadow-[0_16px_48px_-32px_rgba(0,0,0,0.28)] lg:p-5">
          <Button
            ...
            className="w-fit"
            nativeButton={false}
            render={<Link to="/newdrive/$spaceId/{-$folderId}" params={parentParams} />}
          >
            <ArrowLeftIcon data-icon="inline-start" />
            Parent folder
          </Button>
